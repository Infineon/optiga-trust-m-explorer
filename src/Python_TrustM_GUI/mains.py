...
class MainFrame(wx.Frame):
    def __init__(self, parent, title):
        ...
        title_screen = wx.StaticText(
            self, -1, style=wx.ALIGN_CENTER, label="OPTIGA" + "\u1d40\u1d39" + " TRUST M Explorer"
        )
        font = wx.Font(28, wx.ROMAN, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_BOLD)
        title_screen.SetFont(font)

        # TrustM Image
        trustm_image = wx.Image(
            config.IMAGEPATH + "/images/chipp.png", wx.BITMAP_TYPE_PNG
        ).ConvertToBitmap()
        trustm_image = wx.StaticBitmap(self, -1, trustm_image)

        # IFX Logo
        ifx_image = wx.Image(
            config.IMAGEPATH + "/images/250px-Infineon-Logo.png", wx.BITMAP_TYPE_PNG
        )
        ...
        self.button3.SetToolTip(
            wx.ToolTip("Using Trust M Provider and OpenSSL for secure client-server connection")
        )
        self.button4.SetToolTip(
            wx.ToolTip("Integrity and Confidential Protected Update for Key/Metadata/Data")
        )
