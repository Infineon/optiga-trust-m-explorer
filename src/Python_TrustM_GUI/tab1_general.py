...
class Tab_GEN(wx.Panel):
    ...
    def __init__(self, parent):
        ...
        button_meta = wx.Button(
            self, 1, "Read Metadata For All Data Objects", size=wx.Size(350, 50)
        )
        button_meta.SetFont(buttonfont)
        text_pbs_value = wx.StaticText(
            self, -1, "PBS Value:", size=wx.Size(100, 30), style=wx.ALIGN_CENTER
        )
        ...
        button_priv = wx.Button(
            self, 1, "Read Metadata For Private Key Objects", size=wx.Size(350, 50)
        )
        button_priv.SetFont(buttonfont)
        button_metastatus = wx.Button(
            self, 1, "Read Metadata For Common Data Objects", size=wx.Size(350, 50)
        )
        button_metastatus.SetFont(buttonfont)
        button_status = wx.Button(
            self, 1, "Read Data For Common Data Objects", size=wx.Size(350, 50)
        )
        button_status.SetFont(buttonfont)
        button_export_config = wx.Button(
            self, 1, "Generate OPTIGA Trust Configurator Files", size=wx.Size(350, 50)
        )
        ...
        clearimage = wx.Image(
            config.IMAGEPATH + "/images/clear.png", wx.BITMAP_TYPE_PNG
        ).ConvertToBitmap()
        clearbutton = wx.BitmapButton(self, -1, clearimage)

        backimage = wx.Image(
            config.IMAGEPATH + "/images/back.png", wx.BITMAP_TYPE_PNG
        ).ConvertToBitmap()
        ...
        button_meta.SetToolTip(
            wx.ToolTip(
                "Read all data objects metadata for oid: "
                + "0xE0E0-0xE0E3 , 0xE0E8-0xE0E9 , 0xE0EF , 0xE120-0xE123 , 0xE200 , 0xE140 , 0xF1D0-0xF1DB , 0xF1E0-0xF1E1"
            )
        )

        button_data.SetToolTip(
            wx.ToolTip(
                "Read all data object's data for oid: "
                + "0xE0E0-0xE0E3 , 0xE0E8-0xE0E9 , 0xE0EF , 0xE120-0xE123 ,0xE140 , 0xF1D0-0xF1DB , 0xF1E0-0xF1E1"
            )
        )

        button_priv.SetToolTip(
            wx.ToolTip(
                "Read Private data object's metadata for oid: 0xE0F0-0xE0F3 , 0xF1FC-0xE0FD "
            )
        )

        button_metastatus.SetToolTip(
            wx.ToolTip(
                "Read all data objects metadata status for oid: 0xE0C0-0xE0C6 , 0xF1C0-0xF1C2 "
            )
        )

        button_status.SetToolTip(
            wx.ToolTip("Read all data object's status for oid: 0xE0C0-0xE0C6 , 0xF1C0-0xF1C2 ")
        )

        button_export_config.SetToolTip(
            wx.ToolTip(
                "OPTIGA Trust Configurator(OTC) is a tool which can be used to generate customer specific configurations for Infineon Secure Elements. The OTC files generated here can be imported into infineon OPTIGA Trust Configurator to create custom security chip configurations"
            )
        )
    ...
    def OnReset(self, event):
        if os.path.exists(self.pbs_dir):  # Check if the directory exists
            ...
            self.text_display.AppendText(
                "PBS file does not exist. You may proceed to the next step.\n"
            )
        ...
        if os.path.isdir(self.pbs_dir):  # Check if the PBS directory exists
            ...
            try:
                with open(pbsfile_path, "w") as f:
                    f.write(default_pbs)
                self.text_display.AppendText(
                    "pbsfile.txt has been reset to the default PBS value.\n"
                )
            except Exception as e:
    ...
    def OnExportConfig(self, evt):
        ...
        saveFileDialog = wx.DirDialog(
            frame, "Save Config File Folders", "", wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT
        )
        ...
        self.configFilePath = (
            saveFileDialog.GetPath() + "/OPTIGA_Trust_M_V3_SLS32AIA010ML_K_Infineon_Technologies"
        )
        ...
        infoDialog = wx.MessageDialog(
            None,
            "Please import the generated folder into OPTIGA Trust Configurator for final configuration. OPTIGA Trust Configurator can be downloaded from Infineon website.\nPlease wait for several minutes when Exporting the configurations",
            "Generate files in OPTIGA Trust Configurator Tool tabs",
    ...
    def OnExportConfigExec(self):
        ...
        for oid in self.oidList:
            ...
            if oid not in self.noDataOIDList:
                dataToWrite = ""
                commandDataOutput = exec_cmd.execCLI(
                    [config.EXEPATH + "/bin/trustm_data", "-r", targetOID, "-X"]
                )
            ...
            # writing to xml
            commandMetadataOutput = exec_cmd.execCLI(
                [config.EXEPATH + "/bin/trustm_metadata", "-r", targetOID, "-X"]
            )
            ...
            if oid in self.specialOIDList:
                ...
                if readTagIndex > -1:
                    ...
                    readTag = metadataToWrite[
                        readTagIndex : (readTagIndex + (4 + 3 * readTagSize) + 1)
                    ]
...
class Tab_KEY(wx.Panel):
    def __init__(self, parent):
        ...
        self.pubkey = wx.ComboBox(
            self, 1, choices=pubkey_list, style=wx.CB_READONLY, size=wx.Size(180, 30)
        )
        self.pubkey.SetFont(textctrlfont)

        text_keyslot = wx.StaticText(self, 0, "Key Slot")
        self.keyslot = wx.ComboBox(
            self, 1, choices=keyslot_list, style=wx.CB_READONLY, size=wx.Size(170, 30)
        )
        self.keyslot.SetFont(textctrlfont)

        text_trustanc = wx.StaticText(self, 0, "Destination OID:")
        self.trustanc = wx.ComboBox(
            self, 1, choices=Destination_list, style=wx.CB_READONLY, size=wx.Size(170, 30)
        )
        self.trustanc.SetFont(textctrlfont)

        self.button_keymetadata = wx.Button(
            self, 1, "Read Keyslot Metadata ", size=wx.Size(300, 40)
        )
        self.button_keymetadata.SetFont(buttonfont)

        self.button_certmetadata = wx.Button(
            self, 1, "Read Certificate Metadata", size=wx.Size(300, 40)
        )
        ...
        self.filename_input = wx.TextCtrl(
            self, -1, value="testE0E0.crt", style=wx.CB_READONLY, size=(170, 30)
        )
        ...
        self.bindsecret = wx.TextCtrl(
            self, 1, value="E140", style=wx.CB_READONLY, size=wx.Size(170, 30)
        )
        ...
        self.secretfile = wx.TextCtrl(
            self, -1, value="platform_secret.dat", style=wx.CB_READONLY, size=(170, 30)
        )
    ...
    def OnClickSecretFile(self, evt):
        ...
        openFileDialog = wx.FileDialog(
            ...
            "",
            "",
            "All|*.dat;*.crt;*.txt|Binary|*.dat|Secret|*.crt;*.txt",
    ...
    def OnKeyMetadata(self, evt):
        ...
        self.text_display.AppendText(
            "Reading out Metadata of OPTIGA™ TrustM's Key Slot " + keyslot + " ....\n"
        )
    ...
    def OnCertmeta(self, evt):
        ...
        self.text_display.AppendText(
            "Reading out Metadata for OPTIGA™ TrustM's Public Cert " + pubkey + "....\n"
        )
    ...
    def OnCertData(self, evt):
        ...
        self.text_display.AppendText(
            "Reading out data stored in OPTIGA™ TrustM's Public Cert: " + pubkey + "....\n"
        )
    ...
    def OnWriteCert1(self):
        ...
        self.text_display.AppendText(command_output)
        self.text_display.AppendText(
            "'trustm_data -w " + pubkey + "-i " + certfile + " executed \n"
        )
        self.text_display.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")
    ...
    def OnWriteSecret1(self):
        command_output = exec_cmd.createProcess(
            f"python3 hex_to_binary.py {'0102030405060708090A0B0C0D0E0F101112131415161718191A1B1C1D1E1F202122232425262728292A2B2C2D2E2F303132333435363738393A3B3C3D3E3F40'} > platform_secret.dat"
        )
        ...
        self.text_display.AppendText(command_output)
        self.text_display.AppendText(
            "'trustm_data -X -w " + secretoid + "-i " + certfile + " executed \n"
        )
        command_output = exec_cmd.execCLI(
            [
                config.EXEPATH + "/bin/trustm_data",
...
class Tab_APP(wx.Panel):
    def __init__(self, parent):
        ...
        self.data = wx.ComboBox(
            self, -1, choices=dataobject_list, style=wx.CB_READONLY, size=wx.Size(170, 30)
        )
        self.data.SetFont(textctrlfont)

        self.button_metadata = wx.Button(
            self, 1, "Read Metadata of Data Objects ID", size=wx.Size(330, 50)
        )
        ...
        self.button_writedata = wx.Button(
            self, 1, "Write Data into Data Objects ID", size=wx.Size(330, 50)
        )
    ...
    def OnMeta1(self, evt):
        self.text_display.AppendText(
            "Reading out Metadata of OPTIGA™ TrustM's Data Object ID "
            + self.data.GetValue()
            + "....\n"
        )
    ...
    def OnData(self, evt):
        self.text_display.AppendText(
            "\nReading out data stored in OPTIGA™ TrustM's Data Object ID: "
            + self.data.GetValue()
            + "....\n"
        )
...
class Tab_META(wx.Panel):
    def __init__(self, parent):
        ...
        self.sysdata = wx.ComboBox(
            self, 1, choices=sysdatalist, style=wx.CB_READONLY, size=wx.Size(178, -1)
        )
        self.sysdata.SetFont(textctrlfont)

        text_counterobj = wx.StaticText(self, 0, "Counter Objects:")
        self.counterobj = wx.ComboBox(
            self, 1, choices=counter_list, style=wx.CB_READONLY, size=wx.Size(178, -1)
        )
        self.counterobj.SetFont(textctrlfont)

        text_key = wx.StaticText(self, 0, "ECC/RSA/AES key")
        self.key = wx.ComboBox(
            self, 1, choices=key_list, style=wx.CB_READONLY, size=wx.Size(178, -1)
        )
        self.key.SetFont(textctrlfont)

        text_cert = wx.StaticText(self, 0, "Cert")
        self.cert = wx.ComboBox(
            self, 1, choices=cert_list, style=wx.CB_READONLY, size=wx.Size(178, -1)
        )
        self.cert.SetFont(textctrlfont)

        text_gendataobj = wx.StaticText(self, 0, "General OID")
        self.gendataobj = wx.ComboBox(
            self, 1, choices=genoid_list, style=wx.CB_READONLY, size=wx.Size(178, -1)
        )
        self.gendataobj.SetFont(textctrlfont)

        text_bindsecret = wx.StaticText(self, 0, "Binding_Secret:")
        self.bindsecret = wx.ComboBox(
            self, 1, choices=bind_list, style=wx.CB_READONLY, size=wx.Size(178, -1)
        )
        self.bindsecret.SetFont(textctrlfont)

        text_lcs0 = wx.StaticText(self, 0, "Lcs0:4 modes")
        self.lcs0 = wx.ComboBox(
            self, 1, choices=lcs0_list, style=wx.CB_READONLY, size=wx.Size(178, -1)
        )
        self.lcs0.SetFont(textctrlfont)

        text_change = wx.StaticText(self, 0, "Change")
        self.change = wx.ComboBox(
            self, 1, choices=change_list, style=wx.CB_READONLY, size=wx.Size(178, -1)
        )
        self.change.SetFont(textctrlfont)

        text_read = wx.StaticText(self, 0, "Read")
        self.read = wx.ComboBox(
            self, 1, choices=read_list, style=wx.CB_READONLY, size=wx.Size(178, -1)
        )
        self.read.SetFont(textctrlfont)

        text_exe = wx.StaticText(self, 0, "Exe")
        self.exe = wx.ComboBox(
            self, 1, choices=exe_list, style=wx.CB_READONLY, size=wx.Size(178, -1)
        )
        self.exe.SetFont(textctrlfont)

        # general OID data type
        text_oid_datatype = wx.StaticText(self, 0, "General OID data type")
        self.oid_datatype = wx.ComboBox(
            self, 1, choices=datatype_list, style=wx.CB_READONLY, size=wx.Size(178, -1)
        )
        self.oid_datatype.SetFont(textctrlfont)
        #

        text_trust_anchor_oid = wx.StaticText(self, -1, "trust_anchor_oid:")
        self.trust_anchor_oid = wx.ComboBox(
            self, 1, choices=trustoid_list, style=wx.CB_READONLY, size=wx.Size(178, -1)
        )
        self.trust_anchor_oid.SetFont(textctrlfont)

        text_secret_oid = wx.StaticText(self, -1, "secret_oid:")
        self.secret_oid = wx.ComboBox(
            self, 1, choices=secretoid_list, style=wx.CB_READONLY, size=wx.Size(178, -1)
        )
        ...
        button_write_custom_metadata = wx.Button(
            self, 1, "Write Custom Metadata", size=wx.Size(190, -1)
        )
        ...
        gdsizer3.AddMany(
            [
                (datatypesizer, 0, wx.EXPAND),
            ]
        )

        gdsizer4.AddMany(
            [
                (MUDCheckBoxSizer, 0, wx.EXPAND),
            ]
        )
    ...
    def OnLcs0Warning(self):
        if self.MUDCheckBox.GetValue() == False:
            warningDialog = wx.MessageDialog(
                None,
                'Warning: Any manipulation with the lifecycle state might lock the data key/slot permanently. As a safety measure, "MUD Provision" has been enabled. This is NOT REVERSIBLE WITHOUT MUD PROVISION. Continue?',
                "Warning",
    ...
    def OnClickFileName(self, evt):
        ...
        openFileDialog = wx.FileDialog(
            frame, "Open", "", "", "Text|*.txt", wx.FD_OPEN | wx.FD_FILE_MUST_EXIST
        )
        cert_dir = config.IMAGEPATH + "/working_space/"
        openFileDialog.SetDirectory(cert_dir)
    ...
    def OnWriteMetadata(self, evt):
        try:
            ...
            if self.MUDCheckBox.GetValue() == True:
                ...
                self.text_display.AppendText(
                    "Writing Metadata to OPTIGA™ TrustM's Data Object " + DataObject + " ....\n"
                )
    ...
    def OnNoOIDSelected(self):
        infoDialog = wx.MessageDialog(
            None,
            "Select Target OID To Write/Read Metadata",
            "No Target OID Selected",
    ...
    def OnWriteMetadataWithMUDExec(self):
        ...
        TARGET_OID_META = (
            "2010C1020000F00101D80721"
            + self.trust_anchor_oid.GetValue()
            + "FD20"
            + self.secret_oid.GetValue()
        )

        # Step1: Provisioning metadata for Trust Anchor
        self.text_display.AppendText("Provisioning for trust anchor metadata... \n")
        command_output = exec_cmd.createProcess(
            f"python3 hex_to_binary.py {TRUST_ANCHOR_META} > trust_anchor_metadata.bin"
        )
        self.text_display.AppendText(
        ...
        command_output = exec_cmd.execCLI(
            [
                ...
                trust_anchor_oid,
                "-F",
                "trust_anchor_metadata.bin",
            ]
        )
        self.text_display.AppendText(command_output)
        self.text_display.AppendText(
            "'trustm_metadata -w 0x"
            + trust_anchor_oid
            + " -F trust_anchor_metadata.bin'"
            + " executed \n"
        )
        self.text_display.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")
        ...
        command_output = exec_cmd.createProcess(
            f"python3 hex_to_binary.py {PROTECTED_UPDATE_SECRET_META} > protected_update_secret_metadata.bin"
        )
        self.text_display.AppendText(
        ...
        command_output = exec_cmd.execCLI(
            [
                ...
                secret_oid,
                "-F",
                "protected_update_secret_metadata.bin",
            ]
        )
        self.text_display.AppendText(command_output)
        self.text_display.AppendText(
            "'trustm_metadata -w "
            + secret_oid
            + " -F protected_update_secret_metadata.bin' executed \n"
        )
        self.text_display.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")

        ##Set metadata for Target OID
        self.text_display.AppendText(
            "Set metadata protected update for Target OID (Provision for Target OID)... \n"
        )
        command_output = exec_cmd.createProcess(
            f"python3 hex_to_binary.py {TARGET_OID_META} > targetOID_metadata.bin"
        )
        self.text_display.AppendText(
            "'python3 hex_to_binary.py {TARGET_OID_META} > targetOID_metadata.bin' executed \n"
        )
        self.text_display.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")
        self.text_display.AppendText(
            "Writing targetOID_metadata.bin as metadata of Target OID... \n"
        )
        command_output = exec_cmd.execCLI(
            [
                config.EXEPATH + "/bin/trustm_metadata",
                ...
                target_oid,
                "-F",
                "targetOID_metadata.bin",
            ]
        )
        self.text_display.AppendText(command_output)
        self.text_display.AppendText(
            "'trustm_metadata -w " + target_oid + " -F targetOID_metadata.bin' executed \n"
        )
        self.text_display.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")
    ...
    def OnWriteMetadataExec(self):
        ...
        if self.oid_datatype.GetValue() != "-":
            ...
            command_output = exec_cmd.execCLI(
                [config.EXEPATH + "/bin/trustm_metadata", "-w", DataObject, "-F", datatype_file]
            )
            self.text_display.AppendText(command_output)
            self.text_display.AppendText(
                "'trustm_metadata -w " + DataObject + " -F " + datatype_file + "' executed \n"
            )
            self.text_display.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")
    ...
    def OnResetAccess(self, evt):
        try:
            DataObject = self.dataobject
            self.text_display.AppendText(
                "Resetting Metadata Update Description tag for Target OID 0x" + DataObject + "\n"
            )
    ...
    def OnResetAccessExec(self):
        ...
        self.text_display.AppendText(
            "'trustm_metadata -w " + target_oid + " -F mud_reset.bin' executed \n"
        )
        self.text_display.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")
    ...
    def OnSaveMetadata(self, evt):
        ...
        if len(metadataInput) % 2 != 0:
            errorDialog = wx.MessageDialog(
                None, "Invalid Metadata Input", "Error", wx.OK | wx.ICON_ERROR
            )
    ...
    def OnNoMetadataFileSelected(self):
        infoDialog = wx.MessageDialog(
            None,
            "Select Metadata File to Write to Target OID",
    ...
    def OnInvalidCustomMetadata(self):
        infoDialog = wx.MessageDialog(
            None,
            "Double check metadata file to ensure it contains only hex strings",
    ...
    def OnMUDCheckBox(self, evt):
        ...
        if cb.GetValue() == False and (
            self.lcs0.GetValue() == "Operational" or self.lcs0.GetValue() == "Termination"
        ):
            warningDialog = wx.MessageDialog(
                None,
                'Warning: Any manipulation with the lifecycle state might lock the data key/slot permanently. As a safety measure, "MUD Provision" has been enabled. This is NOT REVERSIBLE WITHOUT MUD PROVISION. Continue?',
    ...
    def OnMetadata1(self):
        try:
            ...
            command_output = exec_cmd.execCLI(
                [config.EXEPATH + "/bin/trustm_metadata", "-w", DataObject, "-F", inputfile]
            )
            self.text_display.AppendText(command_output)
            self.text_display.AppendText(
                "'trustm_metadata -w " + DataObject + " -F " + inputfile + "' executed \n"
            )
            self.text_display.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")
...
class Tab_PROV(wx.Panel):
    ...
    def __init__(self, parent):
        ...
        button_gencsr = wx.Button(
            self, 1, "Generate DAC CSR Using Public Key", size=wx.Size(350, 50)
        )
        button_gencsr.SetFont(buttonfont)
        button_gencert = wx.Button(
            self, 1, "Generate DAC Cert Using Public Key", size=wx.Size(350, 50)
        )
        ...
        button_gencert.SetToolTip(
            wx.ToolTip("Generate DAC certificate using public key, Signed by Matter Test PAI ")
        )
    ...
    def OnGenCsr1(self):
        command_output = exec_cmd.execCLI(
            [
                ...
                "request.csr",
                "-config",
                config.EXEPATH + "/scripts/matter_provisioning/test_files/openssl_matter.cnf",
            ]
        )
    ...
    def OnGenCert1(self):
        command_output = exec_cmd.execCLI(
            [
                ...
                "request.csr",
                "-extfile",
                config.EXEPATH + "/scripts/matter_provisioning/test_files/v3.ext",
                "-keyout",
                "-CA",
                config.EXEPATH
                + "/scripts/matter_provisioning/credentials/Matter-Development-PAI-noPID-Cert.pem",
                "-CAkey",
                config.EXEPATH
                + "/scripts/matter_provisioning/credentials/Matter-Development-PAI-noPID-Key.pem",
                "-CAcreateserial",
                "-out",
                "DAC_Cert.pem",
                "-days",
    ...
    def OnWriteCd1(self):
        command_output = exec_cmd.execCLI(
            [
                ...
                "0xf1e0",
                "-i",
                config.EXEPATH + "/scripts/matter_provisioning/credentials/Chip-Test-CD-Cert.bin",
                "-X",
            ]
        )
...
class Tab_MTRPROV(wx.Panel):
    def __init__(self, parent):
        ...
        # gdsizer3
        text_transport_key = wx.StaticText(
            self, -1, "Transport Key:", size=wx.Size(150, 38), style=wx.ALIGN_CENTER
        )
        ...
        self.cdbin_display = wx.TextCtrl(
            self, -1, value="to select .bin or .der file", style=wx.TE_READONLY
        )
        ...
        # gdlcso
        self.Lcsocheckbox = wx.CheckBox(
            self, label="Set to operational (irreversible)", style=wx.CHK_2STATE
        )
    ...
    def OnKeyValue(self, key_type):
        ...
        if not os.path.exists(extracted_bundle):
            wx.MessageBox(
                f"Bundle file is not correctly extracted: {extracted_bundle}",
                "Error",
                wx.OK | wx.ICON_ERROR,
            )
            return None
        ...
        with wx.FileDialog(
            ...
            try:
                ...
                command_output = subprocess.run(
                    ["7z", "x", bundle_path, f"-o{bundle_name}"], capture_output=True, text=True
                )
    ...
    def OnSelectKeyFile(self, event):
        with wx.FileDialog(
            ...
            self.keypath_display.SetValue(
                os.path.basename(key_file_path)
    ...
    def OnExtractKeys(self, extracted_bundle):
        ...
        if command_output.returncode == 0:
            ...
            self.text_display.AppendText(
                f"Error extracting keys archive: \n{command_output.stderr}\n"
            )
            return
    ...
    def OnWriteCD(self, evt):
        if self.cdbin_display.GetValue() == "to select .bin or .der file":
            wx.MessageBox(
                "No file selected. Please select a file first.", "Error", wx.OK | wx.ICON_ERROR
            )
            return

        self.text_display.AppendText("\nWrite CD into 0xf1e0\n")
        command_output = exec_cmd.execCLI(
            [config.EXEPATH + "/bin/trustm_data", "-e", "-w", "0xf1e0", "-i", self.cd_path, "-X"]
        )
        if not command_output:  # Handle empty or error case
            wx.MessageBox(
                "Failed to read the file. Check if it's valid.", "Error", wx.OK | wx.ICON_ERROR
            )
            return
        self.text_display.AppendText(command_output)
    ...
    def OnChipID(self):
        ...
        result = subprocess.run(
            [config.EXEPATH + "/bin/trustm_chipinfo"], capture_output=True, text=True
        )
    ...
    def OnExtractPem(self, key_type):
        ...
        if not os.path.exists(archive_path):
            self.text_display.AppendText(
                f"Error: Archive {key_type} file not found at {archive_path}\n"
            )
            return
        ...
        try:
            ...
            if target_pem_filename not in output_str:
                self.text_display.AppendText(
                    f"Error: {target_pem_filename} not found inside {archive_filename}\n"
                )
                return
        ...
        try:
            ...
            if extract_command is not None and "Error" not in extract_command:
                self.text_display.AppendText(
                    f"Successfully extracted {target_pem_filename} to: {extracted_bundle}\n"
                )
                if key_type == "DACs":
                    self.OnCheckbox("DAC")
                if key_type == "PAI":
                ...
                self.text_display.AppendText(
                    f"Error extracting {target_pem_filename}: {extract_command}\n"
                )
    ...
    def OnWriteDac1(self):
        ...
        command2_output = exec_cmd.execCLI(
            ["openssl", "x509", "-in", dac_pem_path, "-text", "-noout"]
        )
    ...
    def OnWritePai1(self):
        ...
        command2_output = exec_cmd.execCLI(
            ["openssl", "x509", "-in", pai_pem_path, "-text", "-noout"]
        )
        if isinstance(command2_output, bytes):
            command2_output = command2_output.decode("utf-8")
        if command2_output is None or "Error" in command2_output:
    ...
    def OnWriteAll(self, evt):
        ...
        if self.cdbin_display.GetValue() == "to select .bin or .der file":
            wx.MessageBox(
                "No CD binary file selected. Please select a file first.",
                "Error",
                wx.OK | wx.ICON_ERROR,
            )
        ...
        try:
            ...
            try:
                ...
                if isinstance(read_output, bytes):
                    read_output = read_output.decode(
                        "utf-8", errors="ignore"
    ...
    def OnVerifyPAI(self, cert2_output):
        ...
        if not os.path.exists(matter_pai_path):
            exec_cmd.execCLI(
                [config.EXEPATH + "/bin/trustm_cert", "-r", "0xe0e8", "-o", matter_pai_path]
            )

        cert1_output = exec_cmd.execCLI(
            ["openssl", "x509", "-in", matter_pai_path, "-text", "-noout"]
        )
    ...
    def OnVerifyDAC(self, cert2_output):
        ...
        if not os.path.exists(matter_dac_path):
            exec_cmd.execCLI(
                [config.EXEPATH + "/bin/trustm_cert", "-r", "0xe0e0", "-o", matter_dac_path]
            )

        cert1_output = exec_cmd.execCLI(
            ["openssl", "x509", "-in", matter_dac_path, "-text", "-noout"]
        )
        if isinstance(cert1_output, bytes):
            cert1_output = cert1_output.decode("utf-8")
    ...
    def OnLcsocheckboxChanged(self, event):
        if self.Lcsocheckbox.IsChecked():
            wx.MessageBox(
                "Provisioning OIDs to operational mode...", "Info", wx.OK | wx.ICON_INFORMATION
            )
            self.provision_oids()
    ...
    def provision_oids(self):
        ...
        for oid in target_oids:
            try:
                ...
                subprocess.run(
                    cmd_write, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE
                )

                # Verify the change
                cmd_read = ["trustm_metadata", "-r", f"0x{oid}"]
                result = subprocess.run(
                    cmd_read, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE
                )

                wx.MessageBox(
                    f"Successfully provisioned OID 0x{oid}:\n{result.stdout.decode()}",
                    "Success",
                    wx.OK | wx.ICON_INFORMATION,
                )
            except subprocess.CalledProcessError as e:
                wx.MessageBox(
                    f"Error provisioning OID 0x{oid}:\n{e.stderr.decode()}",
                    "Error",
                    wx.OK | wx.ICON_ERROR,
                )
