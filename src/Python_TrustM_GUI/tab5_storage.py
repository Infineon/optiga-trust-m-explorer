...
class Tab5Frame(wx.Frame):
    def __init__(self, parent, title):
        ...
        self.targetoid = wx.ComboBox(
            self, 1, choices=TARGET_OID_list, style=wx.CB_READONLY, size=wx.Size(170, 30)
        )
        self.targetoid.SetFont(textctrlfont)

        text_secretoid = wx.StaticText(self, 0, "Secret OID:")
        self.secretoid = wx.ComboBox(
            self, 1, choices=SECRET_OID_list, style=wx.CB_READONLY, size=wx.Size(170, 30)
        )
        self.secretoid.SetFont(textctrlfont)

        text_secret = wx.StaticText(self, -1, "    Secret:   ")
        self.secret = wx.TextCtrl(
            self,
            value="49C9F492A992F6D4C54F5B12C57EDB27CED224048F25482AA149C9F492A992F649C9F492A992F6D4C54F5B12C57EDB27CED224048F25482AA149C9F492A992F6",
        )
        ...
        # Set tooltips
        provision_button.SetToolTip(
            wx.ToolTip(
                "Provision initial data, metadata and shared secret for HMAC authenticated secure storage access."
            )
        )
        verify_read_button.SetToolTip(
            wx.ToolTip("Verify HMAC SHA256 at SHARED_SECRET_OID and read data in DATA_OBJECT_OID.")
        )
        verify_write_button.SetToolTip(
            wx.ToolTip(
                "Verify HMAC SHA256 at SHARED_SECRET_OID and write new data into DATA_OBJECT_OID."
            )
        )
    ...
    def OnProvision(self):
        ...
        exec_cmd.createProcess(
            f"python3 hex_to_binary.py {SHARED_SECRET_META} > secret_autoref_metadata.bin"
        )
        self.text_display.AppendText(
        ...
        command_output = exec_cmd.execCLI(
            [
                ...
                SECRET_OID,
                "-F",
                "secret_autoref_metadata.bin",
            ]
        )
        self.text_display.AppendText(command_output)
        self.text_display.AppendText(
            "'trustm_metadata -w " + SECRET_OID + " -F secret_autoref_metadata.bin' executed \n"
        )
        self.text_display.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")
        ...
        self.text_display.AppendText(
            "'python3 hex_to_binary.py {SHARED_SECRET} > shared_secret.dat' executed \n"
        )
        command_output = exec_cmd.execCLI(
            [
        ...
        self.text_display.AppendText(command_output)
        self.text_display.AppendText(
            "'trustm_data -e -w " + SECRET_OID + " -i shared_secret.dat' executed \n"
        )
        self.text_display.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")

        self.text_display.AppendText(
            "Set the metadata of 0x$DATA_OBJECT_OID to Auto Change/Read with 0x$SHARED_SECRET_OID.... \n"
        )
        NEW_SECRET_OID = "0323" + self.secretoid.GetValue()
        exec_cmd.createProcess(
            f"python3 hex_to_binary.py {NEW_SECRET_OID} > data_object_auto_metadata.bin"
        )
        self.text_display.AppendText(
            "'python3 hex_to_binary.py {NEW_SECRET_OID} > data_object_auto_metadata.bin' executed \n"
        )
        command_output = exec_cmd.execCLI(
            [
                config.EXEPATH + "/bin/trustm_metadata",
                "-w",
                TARGET_OID,
                "-Cf:data_object_auto_metadata.bin",
                "-Rf:data_object_auto_metadata.bin",
            ]
        )
        self.text_display.AppendText(command_output)
        self.text_display.AppendText(
            "'trustm_metadata -w "
            + TARGET_OID
            + " -Cf:data_object_auto_metadata.bin -Rf:data_object_auto_metadata.bin' executed \n"
        )
        self.text_display.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")
