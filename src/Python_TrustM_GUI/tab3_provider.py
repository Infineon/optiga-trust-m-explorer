...
def ClientProcess(client_log):
    ...
    if client_proc is not None:
        try:
            ...
            client_proc = None
    client_proc = exec_cmd.createProcess2(
...
class Tab_ECC_CS(wx.Panel):
    def __init__(self, parent):
        ...
        button_gen_server_privkey = wx.Button(
            self, -1, "Create Server ECC Private Key", size=(-1, 48)
        )
        ...
        button_gen_client_key_csr = wx.Button(
            self, -1, "Create Client ECC Key and CSR", size=(-1, 48)
        )
        ...
        # Set tooltips
        button_gen_server_privkey.SetToolTip(
            wx.ToolTip("Generate ECC private key using prime256v1 Elliptic Curve.")
        )
        button_gen_server_key_csr.SetToolTip(wx.ToolTip("Generate CSR using EC Key."))
        button_gen_server_cert.SetToolTip(
            wx.ToolTip("Create Server Certificate using Certificate Authority.")
        )

        button_gen_client_key_csr.SetToolTip(
            wx.ToolTip(
                "Create ECC 256 key length with Auth/Enc/Sign usage and Certificate Signing Request using Trust M."
            )
        )
        button_extract_pubkey.SetToolTip(wx.ToolTip("Extract ECC Public Key from CSR."))
        button_gen_client_cert.SetToolTip(
            wx.ToolTip("Create Client Certificate using Certificate Authority.")
        )
    ...
    def OnGenServerCert(self, evt):
        ...
        command_output = exec_cmd.execCLI(
            [
                ...
                "server1.csr",
                "-CA",
                config.CERT_PATH + "/OPTIGA_Trust_M_Infineon_Test_CA.pem",
                "-CAkey",
                config.CERT_PATH + "/OPTIGA_Trust_M_Infineon_Test_CA_Key.pem",
                "-CAcreateserial",
                "-out",
                "server1.crt",
                "-days",
    ...
    def OnGenClientKeyCSR(self):
        command_output = exec_cmd.execCLI(
            [
                ...
                "trustm_provider",
                "-key",
                "0xe0f3:*:NEW:0x03:0x13",
                "-new",
                "-out",
                "client1_e0f3.csr",
                "-subj",
                "/CN=TrustM/O=Infineon/C=SG",
            ]
        )
        ...
        command_output = exec_cmd.execCLI(["openssl", "req", "-in", "client1_e0f3.csr", "-text"])
        self.text_client.AppendText(command_output)
        ...
        try:
            ...
            command_output = subprocess.check_output(
                [
                    ...
                    "-noout",
                    "-out",
                    "client1_e0f3.pub",  # Specify the output file name here
                ],
            ...
            command_output = subprocess.check_output(
                ["openssl", "pkey", "-in", "client1_e0f3.pub", "-pubin", "-text"],
                stderr=subprocess.STDOUT,
                text=True,
            )
            self.text_client.AppendText(
    ...
    def OnStartServer(self, evt):
        ...
        # if server thread is not running
            ...
            openssl_cmd = (
                "openssl s_server -cert server1.crt -key server1_privkey.pem -accept 5000 -verify_return_error -Verify 1 -CAfile "
                + config.CERT_PATH
                + "/OPTIGA_Trust_M_Infineon_Test_CA.pem"
            )
    ...
    def OnStartClient(self, evt):
        if self.client_proc is not None:
            ...
            openssl_cmd = (
                "openssl s_client -servername Server1 -connect localhost:5000 -client_sigalgs ECDSA+SHA256 -provider trustm_provider -provider default -cert client1_e0f3.crt -key 0xe0f3:^ -verify 1 -CAfile "
                + config.CERT_PATH
                + "/OPTIGA_Trust_M_Infineon_Test_CA.pem"
            )
            if self.server_proc is not None:
...
class Tab_RSA_CS(wx.Panel):
    def __init__(self, parent):
        ...
        # instantiate the objects
        button_gen_server_privkey = wx.Button(
            self, -1, "Create Server Private Key and CSR", size=(-1, 48)
        )
        # button_gen_server_key_csr = wx.Button(self, -1, 'Create Server Keys CSR', size = (-1, 48))
        button_gen_server_cert = wx.Button(self, -1, "Create Server Cert", size=(-1, 48))

        button_gen_client_key_csr = wx.Button(
            self, -1, "Create Client RSA Key and CSR", size=(-1, 48)
        )
        # button_extract_pubkey = wx.Button(self, -1, 'Extract Public Key from CSR', size = (-1, 48))
        button_gen_client_cert = wx.Button(self, -1, "Create Client Cert", size=(-1, 48))
        ...
        button_gen_client_key_csr.SetToolTip(
            wx.ToolTip(
                "Create RSA 2048 key length with Auth/Enc/Sign usage and Certificate Signing Request using Trust M."
            )
        )
    ...
    def OnStartServer(self, evt):
        ...
        # if server thread is not running
            ...
            openssl_cmd = (
                "openssl s_server -tls1_2 -cert server2.crt -key server2_privkey.pem -accept 5001 -verify_return_error -Verify 1 -CAfile "
                + config.CERT_PATH
                + "/OPTIGA_Trust_M_Infineon_Test_CA.pem -sigalgs RSA+SHA256"
            )
            #             openssl_cmd="echo Hello World!"
            self.server_proc = exec_cmd.createProcess(openssl_cmd)
            self.Server_thread_active_flag = 1
            ...
            # this message is sent first
            wx.CallAfter(
                Publisher.sendMessage, "RSA_Server_Text", msg="\n\n" + openssl_cmd + "\n\n"
            )
    ...
    def OnStartClient(self, evt):
        if self.client_proc is not None:
            ...
            openssl_cmd = (
                "openssl s_client -tls1_2 -servername Server2 -connect localhost:5001 -client_sigalgs RSA+SHA256 -provider trustm_provider -provider default -cert client2_rsa.crt -key 0xe0fc:^ -CAfile "
                + config.CERT_PATH
                + "/OPTIGA_Trust_M_Infineon_Test_CA.pem -verify 1"
            )
            if self.server_proc is not None:
                self.client_proc = exec_cmd.createProcess(openssl_cmd)
                ...
                c_thread.start()
                wx.CallAfter(
                    Publisher.sendMessage, "RSA_Client_Text", msg="\n\n" + openssl_cmd + "\n\n"
                )
            else:
                wx.CallAfter(
                    Publisher.sendMessage, "RSA_Client_Text", msg="Server is not active..\n"
                )
...
class Tab_RSA_MISC(wx.Panel):
    ...
    def OnGenKey(self):
        ...
        self.command_out.AppendText(
            f"'openssl pkey -provider trustm_provider -in 0xe0fd:*:NEW:0x42:0x13 -pubout -out e0fd_pub.pem' executed \n"
        )
    ...
    def OnEnc(self):
        ...
        self.command_out.AppendText(
            "'openssl pkeyutl -provider trustm_provider -inkey 0xe0fd:^ -encrypt -in input_data.txt -out mycipher' executed \n"
        )
    ...
    def OnVerify(self, evt):
        ...
        self.command_out.AppendText(
            "'openssl pkeyutl -pubin -inkey e0fd_pub.pem -verify -rawin -in input_data.txt -sigfile mysig' executed \n"
        )
        self.command_out.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")
...
class Tab_ECC_MISC(wx.Panel):
    ...
    def OnGenKey(self):
        ...
        self.command_out.AppendText(
            f"'openssl pkey -provider trustm_provider -in 0xe0f3:*:NEW:0x03:0x33 -pubout -out e0f3_pub.pem' executed \n"
        )
    ...
    def OnVerify(self, evt):
        ...
        self.command_out.AppendText(command_output.decode("utf-8", errors="ignore"))
        self.command_out.AppendText(
            "'openssl pkeyutl -pubin -inkey e0f3_pub.pem -verify -rawin -in input_data.txt -sigfile mysig' executed \n"
        )
        self.command_out.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")
...
class Tab_RNG(wx.Panel):
    def __init__(self, parent):
        ...
        self.enctype = wx.ComboBox(
            self, 1, choices=enctypelist, size=wx.Size(170, 30), style=wx.CB_READONLY
        )
    ...
    def OnGenRNG(self, evt):
        ...
        self.command_display.AppendText(
            "Generating Random Number in " + enctype + " encoding....\n\n"
        )
    ...
    def OnGenRNG1(self):
        ...
        try:
            ...
            self.command_display.AppendText(
                "\n'openssl rand -provider -trustm_provider -"
                + enctype
                + " "
                + str(no_bytes)
                + " executed'\n"
