...
keydata_list = [
    "ecc_secp256r1_test.pem",
    "ecc_secp384r1_test.pem",
    "ecc_secp521r1_test.pem",
    "ecc_brainpool_p256r1_test.pem",
    "ecc_brainpool_p384r1_test.pem",
    "ecc_brainpool_p512r1_test.pem",
]
aesdata_list = [
    "aes_key_128.txt",
    "aes_key_192.txt",
...
step1_list = [
    "Step1: Set Lcso=0x03(Init) ResetType=0x01(Keep TargetData)",
    "Step1: Set Lcso=0x03(Init) ResetType=0x11(Wipe TargetData)",
]
...
class Tab_MetaConfidentialUpdate(wx.Panel):
    def __init__(self, parent):
        ...
        title = wx.StaticText(
            ...
            label="Metadata Integrity Confidential Protected Update",
        )
        ...
        self.secret2path = (
            config.EXEPATH + "/ex_protected_update_data_set/samples/confidentiality/secret.txt"
        )
        text_secret2 = wx.StaticText(self, 0, "secret:")
        self.secret2 = wx.TextCtrl(
            self, 1, value="secret.txt", style=wx.CB_READONLY, size=wx.Size(178, 30)
        )
        ...
        self.secret1 = wx.TextCtrl(
            self, 1, value="secret.txt", style=wx.CB_READONLY, size=wx.Size(178, 30)
        )
        self.secret1.SetFont(textctrlfont)
        text_secret = wx.StaticText(self, 0, "protected_update:")
        self.secret = wx.ComboBox(
            self, 1, choices=secret_list, style=wx.CB_READONLY, size=wx.Size(178, -1)
        )
        ...
        self.sign_algo = wx.ComboBox(
            self, 1, choices=sign_algo_list, style=wx.CB_READONLY, size=wx.Size(178, -1)
        )
        self.sign_algo.SetFont(textctrlfont)

        self.metapath = (
            config.EXEPATH + "/ex_protected_update_data_set/samples/payload/metadata/metadata.txt"
        )
        text_metadata = wx.StaticText(self, 0, "metadata:")
        self.metadata = wx.TextCtrl(
            self, 1, value="metadata.txt", style=wx.CB_READONLY, size=wx.Size(178, 30)
        )
        ...
        self.priv_key = wx.TextCtrl(
            self, -1, value="sample_ec_256_priv.pem", style=wx.CB_READONLY, size=(178, 30)
        )
        ...
        self.trust_anchor_oid = wx.ComboBox(
            self, 1, choices=meta_trust_anchor_oid_list, style=wx.CB_READONLY, size=wx.Size(178, -1)
        )
        self.trust_anchor_oid.SetFont(textctrlfont)
        text_target_oid = wx.StaticText(self, -1, "target_oid:")
        self.target_oid = wx.ComboBox(
            self, 1, choices=meta_target_oid_list, style=wx.CB_READONLY, size=wx.Size(178, -1)
        )
        self.target_oid.SetFont(textctrlfont)
        text_secret_oid = wx.StaticText(self, -1, "secret_oid:")
        self.secret_oid = wx.ComboBox(
            self, 1, choices=meta_secret_oid_list, style=wx.CB_READONLY, size=wx.Size(178, -1)
        )
        ...
        self.trust_anchor_cert = wx.TextCtrl(
            self, 1, value="sample_ec_256_cert.pem", style=wx.CB_READONLY, size=wx.Size(178, 30)
        )
        ...
        button_read_objects_metadata = wx.Button(
            self, 1, "Read Objects Metadata", size=wx.Size(270, -1)
        )
        ...
        self.step1_combo_box = wx.ComboBox(
            self, 1, choices=step1_list, style=wx.CB_READONLY, size=wx.Size(554, -1)
        )
        self.step1_combo_box.SetFont(buttonfont)
        self.button_step1 = wx.Button(
            self, -1, "Step1: Provisioning for All OIDs", size=wx.Size(270, -1)
        )
        ...
        serverimage = (
            wx.Image(config.IMAGEPATH + "/images/server.png", wx.BITMAP_TYPE_PNG)
            .Scale(110, 110, wx.IMAGE_QUALITY_HIGH)
            .ConvertToBitmap()
        )
        ...
        lockedarrow = wx.Image(
            config.IMAGEPATH + "/images/LockedArrow.png", wx.BITMAP_TYPE_PNG
        ).ConvertToBitmap()
        lockedarrowimage = wx.StaticBitmap(self, -1, lockedarrow)

        chipimage = wx.Image(
            config.IMAGEPATH + "/images/chipp3.png", wx.BITMAP_TYPE_PNG
        ).ConvertToBitmap()
        ...
        # Set tooltips
        self.button_step1.SetToolTip(
            wx.ToolTip(
                "Provision metadata for Trust Anchor OID, Protected Update Secret OID and Target OID.\n"
        ...
        button_step2.SetToolTip(
            wx.ToolTip("Generate the correct manifest and fragment for protected update.")
        )
    ...
    def OnSelectTargetOid(self, evt):
        if (
            self.target_oid.GetValue() == self.secret_oid.GetValue()
            and self.target_oid.GetValue() != "-"
        ):
            infoDialog = wx.MessageDialog(
                None,
                "Target OID and Secret OID cannot be the same",
                "Duplicated Target and Secret OID",
    ...
    def OnSelectSecretOid(self, evt):
        if (
            self.secret_oid.GetValue() == self.target_oid.GetValue()
            and self.secret_oid.GetValue() != "-"
        ):
            infoDialog = wx.MessageDialog(
                None,
                "Target OID and Secret OID cannot be the same",
    ...
    def OnNoOidSelected(self):
        infoDialog = wx.MessageDialog(
            None,
            "Please select target and/or secret OID",
            "No OID selected",
            wx.OK | wx.ICON_INFORMATION,
        )
        infoDialog.ShowModal()
    ...
    def OnClickFileName(self, evt):
        ...
        openFileDialog = wx.FileDialog(
            ...
            "",
            "",
            "All|*.crt;*.pem|Certificate|*.crt;*.pem",
            wx.FD_OPEN | wx.FD_FILE_MUST_EXIST,
        )
    ...
    def OnClickSecret(self, evt):
        ...
        openFileDialog = wx.FileDialog(
            ...
            "",
            "",
            "All|*.crt;*.txt|Certificate|*.crt;*.txt",
            wx.FD_OPEN | wx.FD_FILE_MUST_EXIST,
        )
    ...
    def OnSetup(self):
        ...
        self.text_display.AppendText(
            "'trustm_cert -w 0x" + trust_anchor_oid + " -i " + certfile + "'" + " executed \n"
        )
        self.text_display.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")
        ...
        self.text_display.AppendText(
            "'$PROTECTED_UPDATE_SECRET xxd -r -p > protected_update_secret.dat' executed \n"
        )
        self.text_display.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")
        ...
        command_output = exec_cmd.createProcess(
            "echo "
            + PROTECTED_UPDATE_SECRET_META
            + " | xxd -r -p > protected_update_secret_metadata.bin"
        )
        self.text_display.AppendText(
            "'$PROTECTED_UPDATE_SECRET_META xxd -r -p > protected_update_secret_metadata.bin' executed \n"
        )
        self.text_display.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")
    ...
    def OnSetupTempDecom(self):
        ...
        self.text_display.AppendText(
            "'$TARGET_OID_META | xxd -r -p > targetOID_metadata.bin' executed \n"
        )
        self.text_display.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")
        self.text_display.AppendText(
        ...
        command_output = exec_cmd.execCLI(
            [config.EXEPATH + "/bin/trustm_metadata", "-w", target_oid, "-I"]
        )
        self.text_display.AppendText(command_output)
    ...
    def OnSetupWipeData(self):
        ...
        # Set metadata for Target OID
        self.text_display.AppendText(
            "Set protected update for Target OID (Provision for Target OID)... \n"
        )
        command_output = exec_cmd.createProcess(
        ...
        self.text_display.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")
        self.text_display.AppendText(
            "Writing targetOID_metadata_permanent.bin as metadata of Target OID... \n"
        )
        command_output = exec_cmd.execCLI(
            [
                config.EXEPATH + "/bin/trustm_metadata",
                ...
                target_oid,
                "-F",
                "targetOID_metadata_permanent.bin",
            ]
        )
        self.text_display.AppendText(command_output)
        self.text_display.AppendText(
    ...
    def OnGenManifest(self, evt):
        ...
        if secret_list_value == "Integrity":
            command_output = exec_cmd.createProcess(
                config.EXEPATH
                + "/ex_protected_update_data_set/Linux/bin/trustm_protected_update_set "
                + payload_version
                + " "
                + trust_anchor_oid
                + " "
        ...
        manifestData = ""
        for manifestLines in lines[
            manifestStartLine + 3 : manifestStartLine + 3 + manifestDataLineLimit
        ]:
        ...
        exec_cmd.createProcess(f"python3 hex_to_binary.py {manifestData} > manifest.dat")
        exec_cmd.createProcess(f"python3 hex_to_binary.py {fragmentData} > fragment.dat")
    ...
    def OnProtectedUpdate(self):
        ...
        command_output = exec_cmd.execCLI(
            [
                config.EXEPATH + "/bin/trustm_protected_update",
                "-k",
                target_oid,
                "-m",
                "manifest.dat",
        ...
        self.text_display.AppendText(command_output)
        self.text_display.AppendText(
            "'trustm_protected_update -k "
            + target_oid
            + " -m manifest.dat -f fragment.dat' executed \n"
        )
        self.text_display.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")
...
class Tab_KeyConfidentialUpdate(wx.Panel):
    def __init__(self, parent):
        ...
        title = wx.StaticText(
            self, -1, style=wx.ALIGN_CENTER, label="ECC Key Integrity Confidential Protected Update"
        )
        font = wx.Font(20, wx.ROMAN, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_BOLD)
        title.SetFont(font)
        ...
        text_secret2 = wx.StaticText(self, 0, "secret:")
        self.secret2 = wx.TextCtrl(
            self, 1, value="secret.txt", style=wx.CB_READONLY, size=wx.Size(178, -1)
        )
        self.secret2.SetFont(textctrlfont)
        text_keylength = wx.StaticText(self, 0, "key_length:")
        self.keylength = wx.ComboBox(
            self, 1, choices=keylength, style=wx.CB_READONLY, size=wx.Size(178, -1)
        )
        ...
        text_secret = wx.StaticText(self, 0, "protected_update:")
        self.secret = wx.ComboBox(
            self, 1, choices=confidential_list, style=wx.CB_READONLY, size=wx.Size(178, -1)
        )
        self.secret.SetFont(textctrlfont)
        text_payload_version = wx.StaticText(self, 0, "payload_version:")
        ...
        self.keypath = (
            config.EXEPATH + "/ex_protected_update_data_set/samples/payload/key/ecc256test_priv.pem"
        )
        text_keydata = wx.StaticText(self, 0, " privkey_data:")
        self.keydata = wx.TextCtrl(
            self, 1, value="ecc256test_priv.pem", style=wx.CB_READONLY, size=wx.Size(178, -1)
        )
        self.keydata.SetFont(textctrlfont)

        # added a new input space to include the pub key file
        self.pubkeypath = (
            config.EXEPATH + "/ex_protected_update_data_set/samples/payload/key/ecc256test_pub.der"
        )
        pubkey_filename = os.path.basename(self.pubkeypath)
        text_pubkeydata = wx.StaticText(self, 0, "pubkey_data:")
        self.pubkeydata = wx.TextCtrl(
            self, 1, value=pubkey_filename, style=wx.CB_READONLY, size=wx.Size(178, -1)
        )
        ...
        text_trust_anchor_oid = wx.StaticText(self, -1, "trust_anchor_oid:")
        self.trust_anchor_oid = wx.ComboBox(
            self, 1, choices=key_trust_anchor_oid_list, style=wx.CB_READONLY, size=wx.Size(178, -1)
        )
        self.trust_anchor_oid.SetFont(textctrlfont)

        text_target_oid = wx.StaticText(self, -1, "target_oid_ECC:")
        self.target_oid = wx.ComboBox(
            self, 1, choices=key_target_oid_list, style=wx.CB_READONLY, size=wx.Size(178, -1)
        )
        self.target_oid.SetFont(textctrlfont)
        ...
        self.button_step1 = wx.Button(
            self, -1, "Step1:Provision for all OIDs", size=wx.Size(270, -1)
        )
        self.button_step1.SetFont(buttonfont)
        ...
        # Add Objects to leftsizer
        leftsizer.Add(picturesizer, 0, wx.TOP, 85)
        leftsizer.AddSpacer(120)
    ...
    def OnClickPubKeydata(self, evt):
        ...
        openFileDialog = wx.FileDialog(
            ...
            "",
            "",
            "All|*.crt;*.der|Certificate|*.crt;*.der",
            wx.FD_OPEN | wx.FD_FILE_MUST_EXIST,
        )
    ...
    def OnSetup(self):
        ...
        self.text_display.AppendText(command_output)
        self.text_display.AppendText(
            "'trustm_cert -w " + trust_anchor_oid + " -i " + certfile + "'" + " executed \n"
        )
        self.text_display.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")
        ...
        self.text_display.AppendText(command_output)
        self.text_display.AppendText(
            "'trustm_data -e -w " + secret_oid + " -i protected_update_secret.dat'" + " executed \n"
        )
        self.text_display.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")
    ...
    def OnGenManifest(self, evt):
        ...
        exec_cmd.createProcess(f"python3 hex_to_binary.py {manifest} > manifest.dat")
        exec_cmd.createProcess(f"python3 hex_to_binary.py {fragment} > fragment.dat")
        self.text_display.AppendText("\n\nmanifest.dat and fragment.dat generated\n")
        self.text_display.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")
    ...
    def OnProtectedUpdate(self):
        ...
        if self.keylength.GetSelection() == 2 or self.keylength.GetSelection() == 5:
            pubkey_oid = (
                "0x" + ext_pubkey_oid_list[selected_index]
            )  # select corresponding pubkey oid based on index
        ...
        command_output = exec_cmd.execCLI(
            [
                config.EXEPATH + "/bin/trustm_protected_update_ecckey",
                "-k",
                target_oid,
                "-f",
                "fragment.dat",
        ...
        self.text_display.AppendText(command_output)
        self.text_display.AppendText(
            "'trustm_protected_update_ecckey -k "
            + target_oid
            + " -m manifest.dat -f fragment.dat' executed \n"
        )
        command_output2 = exec_cmd.execCLI(
            [
                config.EXEPATH + "/bin/trustm_data",
                "-w",
                pubkey_oid,
    ...
    def OnResetAccess(self):
        ...
        target_oid = "0x" + self.target_oid.GetValue()
        exec_cmd.createProcess(f"python3 hex_to_binary.py {RESET_ACCESS_META} > access_reset.bin")
        command_output = exec_cmd.execCLI(
            [
                "python3",
                "emulator.py",
        ...
        self.text_display.AppendText(
            "'trustm_metadata -w " + target_oid + " -F access_reset.bin' executed \n"
        )
        self.text_display.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")
...
class Tab_AesConfidentialUpdate(wx.Panel):
    def __init__(self, parent):
        ...
        title = wx.StaticText(
            self, -1, style=wx.ALIGN_CENTER, label="AES Key Integrity Confidential Protected Update"
        )
        font = wx.Font(20, wx.ROMAN, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_BOLD)
        title.SetFont(font)
        # title.SetMinSize((200, 30))
        ...
        self.keypath = (
            config.EXEPATH + "/ex_protected_update_data_set/samples/payload/key/aes_key_128.txt"
        )
        text_keydata = wx.StaticText(self, 0, "key_data:")
        self.keydata = wx.TextCtrl(
            self, 1, value="aes_key_128.txt", style=wx.CB_READONLY, size=wx.Size(178, -1)
        )
        self.keydata.SetFont(textctrlfont)
        ...
        self.target_oid = wx.ComboBox(
            self, 1, choices=aes_target_oid_list, style=wx.CB_READONLY, size=wx.Size(178, -1)
        )
        self.target_oid.SetFont(textctrlfont)
        text_secret_oid = wx.StaticText(self, -1, "secret_oid:")
        self.secret_oid = wx.ComboBox(
            self, 1, choices=key_secret_oid_list, style=wx.CB_READONLY, size=wx.Size(178, -1)
        )
        self.secret_oid.SetFont(textctrlfont)
        ...
        text_trust_anchor_cert = wx.StaticText(self, -1, "trust_anchor_cert:")
        self.trust_anchor_cert = wx.TextCtrl(
            self, 1, "sample_ec_256_cert.pem", style=wx.CB_READONLY, size=wx.Size(178, 30)
        )
        self.trust_anchor_cert.SetFont(textctrlfont)
        button_step2 = wx.Button(self, 1, "Step2: Generate Manifest", size=wx.Size(270, -1))
        button_step2.SetFont(buttonfont)
    ...
    def OnProtectedUpdate(self):
        ...
        command_output = exec_cmd.execCLI(
            [
                config.EXEPATH + "/bin/trustm_protected_update_aeskey",
                "-k",
                target_oid,
                "-f",
                "fragment.dat",
...
class Tab_RsaConfidentialUpdate(wx.Panel):
    def __init__(self, parent):
        ...
        title = wx.StaticText(
            self, -1, style=wx.ALIGN_CENTER, label="RSA Key Integrity Confidential Protected Update"
        )
        font = wx.Font(20, wx.ROMAN, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_BOLD)
        title.SetFont(font)
        ...
        self.keypath = (
            config.EXEPATH
            + "/ex_protected_update_data_set/samples/payload/key/rsa2048test_priv.pem"
        )
        text_keydata = wx.StaticText(self, 0, "privkey_data:")
        self.keydata = wx.TextCtrl(
            self, 1, value="rsa2048test_priv.pem", style=wx.CB_READONLY, size=wx.Size(178, -1)
        )
        self.keydata.SetFont(textctrlfont)

        self.pubkeypath = (
            config.EXEPATH + "/ex_protected_update_data_set/samples/payload/key/rsa2048test_pub.der"
        )
        pubkey_filename = os.path.basename(self.pubkeypath)
        text_pubkeydata = wx.StaticText(self, 0, "pubkey_data:")
        self.pubkeydata = wx.TextCtrl(
        ...
        self.target_oid = wx.ComboBox(
            self, 1, choices=rsa_target_oid_list, style=wx.CB_READONLY, size=wx.Size(178, -1)
        )
        self.target_oid.SetFont(textctrlfont)
        text_secret_oid = wx.StaticText(self, -1, "secret_oid:")
        self.secret_oid = wx.ComboBox(
            self, 1, choices=key_secret_oid_list, style=wx.CB_READONLY, size=(178, 30)
        )
        self.secret_oid.SetFont(textctrlfont)
        ...
        button_read_objects_metadata.SetToolTip(
            wx.ToolTip("Read metadata of Trust Anchor OID, Secret OID and Target OID.")
        )
        button_step2.SetToolTip(
            wx.ToolTip("Generate the correct manifest and fragment for protected update.")
        )
    ...
    def OnProtectedUpdate(self):
        ...
        command_output = exec_cmd.execCLI(
            [
                config.EXEPATH + "/bin/trustm_protected_update_rsakey",
                "-k",
                target_oid,
                "-f",
                "fragment.dat",
        ...
        self.text_display.AppendText(command_output)
        self.text_display.AppendText(
            "'trustm_protected_update_rsakey -k "
            + target_oid
            + " -m manifest.dat -f fragment.dat' executed \n"
        )
        command_output2 = exec_cmd.execCLI(
...
class Tab_DataUpdate(wx.Panel):
    def __init__(self, parent):
        ...
        title = wx.StaticText(
            self, -1, style=wx.ALIGN_CENTER, label="Data Integrity Confidential Protected Update"
        )
        font = wx.Font(20, wx.ROMAN, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_BOLD)
        title.SetFont(font)
        # title.SetMinSize((200, 30))
        ...
        self.datapath = (
            config.EXEPATH + "/ex_protected_update_data_set/samples/payload/data/hex_data.txt"
        )
        text_data = wx.StaticText(self, 0, "data:")
        self.data = wx.TextCtrl(
            self, 1, value="type3_data.txt", style=wx.CB_READONLY, size=wx.Size(178, -1)
        )
        self.data.SetFont(textctrlfont)

        # data types to accomondate both ascii and hex
        datatype_text = wx.StaticText(self, 0, "data type:")
        self.datatype = wx.ComboBox(
        ...
        text_trust_anchor_oid = wx.StaticText(self, -1, "trust_anchor_oid:")
        self.trust_anchor_oid = wx.ComboBox(
            self, 1, choices=data_trust_anchor_oid_list, style=wx.CB_READONLY, size=wx.Size(178, -1)
        )
        self.trust_anchor_oid.SetFont(textctrlfont)
        text_target_oid = wx.StaticText(self, -1, "target_oid:")
        self.target_oid = wx.ComboBox(
            self, 1, choices=data_target_oid_list, style=wx.CB_READONLY, size=wx.Size(178, -1)
        )
        self.target_oid.SetFont(textctrlfont)
        text_secret_oid = wx.StaticText(self, -1, "secret_oid:")
        self.secret_oid = wx.ComboBox(
            self, 1, choices=data_secret_oid_list, style=wx.CB_READONLY, size=wx.Size(178, -1)
        )
        self.secret_oid.SetFont(textctrlfont)
    ...
    def OnGenManifest(self, evt):
        ...
        try:
            manifestSize = int(
                lines[manifestStartLine][MANIFEST_SIZE_INDEX : MANIFEST_SIZE_INDEX + 3]
            )
        except IndexError:
            # raise info dialog here
            infoDialog = wx.MessageDialog(
                None,
                "Double check input data file",
        ...
        for line in lines:
            if "Fragment number" in line:
                ...
                if self.fragmentNumber > MAX_NUM_OF_FRAGMENTS:
                    infoDialog = wx.MessageDialog(
                        None,
                        "Please limit fragment numbers than 4 fragments",
        ...
        for x in range(self.fragmentNumber):
            ...
            exec_cmd.createProcess(
                f"python3 hex_to_binary.py {fragmentData[x]} > fragment_" + str(x) + ".dat"
            )
    ...
    def OnProtectedUpdate(self):
        ...
        try:
            if self.fragmentNumber == 1:
                command_output = exec_cmd.execCLI(
                    [
                        config.EXEPATH + "/bin/trustm_protected_update_data",
                        "-k",
                        target_oid,
                        "-m",
                        "manifest.dat",
                ...
                self.text_display.AppendText(
                    "'trustm_protected_update -k "
                    + target_oid
                    + " -m manifest.dat -f fragment_0.dat' executed \n"
                )
                self.text_display.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")
            ...
            if self.fragmentNumber == 2:
                ...
                self.text_display.AppendText(
                    "'trustm_protected_update -k "
                    + target_oid
                    + " -m manifest.dat -c fragment_0.dat -f fragment_1.dat' executed \n"
                )
                self.text_display.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")
            ...
            if self.fragmentNumber == 3:
                ...
                self.text_display.AppendText(
                    "'trustm_protected_update -k "
                    + target_oid
                    + " -m manifest.dat -c fragment_0.dat -d fragment_1.dat -f fragment_2.dat' executed \n"
                )
                self.text_display.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")
            ...
            infoDialog = wx.MessageDialog(
                None,
                "Please run Step 2: Generate Manifest",
